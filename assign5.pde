...
void initClocks(){
  ...
  for(int i = 0; i < clockX.length; i++){
    ...
    for(int j = 0; j < clockX.length; j++){
      if(clockX[i] == cabbageX[j] && clockY[i] == cabbageY[j]){
        i--;
      }
    }
  }
...
int getEnemyIndexByRow(int row){				// Requirement #6
        ...
        if(soldierY[1] == (row+5)*80){
         return 1;
        }else if(soldierY[2] == (row+5)*80){
         return 2;
        }else if(soldierY[3] == (row+5)*80){
         return 3;
        }else if(soldierY[4] == (row+5)*80){
         return 4;
        }else if(soldierY[5] == (row+5)*80){
         return 5;
        }else if(soldierY[0] == (row+5)*80){
         return 0;
        }else{
         return -1;
        }      
